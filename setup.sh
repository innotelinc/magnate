...
# Vault, KV v2, hosted by Cerulean); this repo owns the `magnate` path. Set
# VAULT_ADDR / VAULT_TOKEN_FILE / VAULT_PREFIX / VAULT_PATH in .env, and any
# value there may be a `vault://cerulean/magnate#KEY` reference instead of the
# secret itself. There is no local secret service to provision, and no runtime
# resolver here — materialize a reference into .env with
# scripts/vault-migrate.py before starting the stack.
if grep -qE '^VAULT_ADDR=.+' .env 2>/dev/null; then
  echo ">> Cerulean Vault configured — write resolved values, not references, into .env"
else
